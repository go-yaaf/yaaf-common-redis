...
func main() {
	...
	NewRedisPublisher(redisUri).Name("water_meter_1").Queue("water").Duration(time.Hour).Interval(time.Second).Start(wg)
	NewRedisPublisher(redisUri).Name("water_meter_2").Queue("water").Duration(time.Hour).Interval(time.Second * 2).Start(wg)
